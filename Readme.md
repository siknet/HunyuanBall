...
适用场景：
- 给「沉浸式翻译」提供本地翻译API用于翻译网页全文、油管字幕等
- 提供本地快速翻译Web界面
- 不占显存，不依赖显卡（CPU 模式）

关于**腾讯混元翻译模型**：Hunyuan-MT-1.5-1.8B模型
>Tencent-HY-MT1.5-1.8B支持33个语种互译以及5种民汉/方言，除了中文、英语、日语等常见语种，也包含捷克语、马拉地语、爱沙尼亚语、冰岛语等小语种。HY-MT1.5-1.8B主要面向手机等消费级设备场景，经过量化，支持端侧直接部署和离线实时翻译，仅需1GB内存即可流畅运行，并且在参数量极小的前提下，效果超过了大部分商用翻译API。
>
>
...
1. 把模型文件（`.gguf`）放到 `models/` 目录（模型下载地址：https://huggingface.co/tencent/HY-MT1.5-1.8B-GGUF/blob/main/HY-MT1.5-1.8B-Q6_K.gguf，可选Q4/Q6/Q8）  
...
经测试，纯CPU模式下内存占用1.2Gb-1.5Gb左右，翻译过程额外占用300Mb内存，第一次翻译响应时间在1.5秒-3秒内，之后会更快一些。测试代码详见Repo中的bench.ps1文件。
...
* 沉浸式翻译里：
  ...
  * 每秒最大请求数：建议 1或者2
  * 每次请求最大文本长度：600 左右更稳
  * 每次请求最大段落数：1或者2（速度最快，也最稳定）

如果你机器配置较高，可以相应调高对应的参数并改用更高级的模型：
[Q6量化模型](https://huggingface.co/tencent/HY-MT1.5-1.8B-GGUF/blob/main/HY-MT1.5-1.8B-Q6_K.gguf "Q6量化模型")
[Q8量化模型](https://huggingface.co/tencent/HY-MT1.5-1.8B-GGUF/blob/main/HY-MT1.5-1.8B-Q8_0.gguf "Q8量化模型")
...
* 沉浸式翻译里：
  ...
  * 每秒最大请求数：4
  * 每次请求最大文本长度：1000 左右更稳
  * 每次请求最大段落数：4（速度最快，也最稳定）
...
* 类型：布尔（true/false）
* 作用：运行 `HYBall.exe` 后自动载入模型并开始提供 API 服务
...
* 类型：布尔
* 作用：当模型加载完成并且 API 可用时，自动打开本地 WebUI 网页翻译 页面
